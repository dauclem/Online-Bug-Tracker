...
$t_install_state = gpc_get_int( 'install', 2 );
?>
<html>
<head>
...
require __DIR__.'/../../../../private/setup.vars.php';
...
if( $t_config_exists ) {
	...
	if( CONFIGURED_PASSWORD == $f_db_password ) {
		$f_db_password = config_get( 'db_password', $config['pdo_password'] );
	}
}
$f_admin_username = gpc_get( 'admin_username', $config['pdo_username'] );
$f_admin_password = gpc_get( 'admin_password', $config['pdo_password'] );
